...
const InteractiveDotBackground = () => {
  ...
  const INTERACTION_RADIUS_SQ =
    CANVAS_CONFIG.INTERACTION_RADIUS * CANVAS_CONFIG.INTERACTION_RADIUS;
  ...
  const animateDots = useCallback(() => {
    ...
    if (mouseX !== null && mouseY !== null) {
      ...
      const searchRadius = Math.ceil(
        CANVAS_CONFIG.INTERACTION_RADIUS / GRID_CELL_SIZE
      );
    ...
    dots.forEach((dot, index) => {
      ...
      if (
        ...
        dot.currentOpacity <= CANVAS_CONFIG.BASE_OPACITY_MIN
      ) {
        dot.opacitySpeed = -dot.opacitySpeed;
        dot.currentOpacity = Math.max(
          CANVAS_CONFIG.BASE_OPACITY_MIN,
          Math.min(dot.currentOpacity, CANVAS_CONFIG.BASE_OPACITY_MAX)
        );
        dot.targetOpacity =
          Math.random() *
            (CANVAS_CONFIG.BASE_OPACITY_MAX -
              CANVAS_CONFIG.BASE_OPACITY_MIN) +
          CANVAS_CONFIG.BASE_OPACITY_MIN;
      }
      ...
      if (
        ...
        if (distSq < INTERACTION_RADIUS_SQ) {
          ...
          interactionFactor = Math.max(
            0,
            1 - distance / CANVAS_CONFIG.INTERACTION_RADIUS
          );
      ...
      const finalOpacity = Math.min(
        1,
        dot.currentOpacity + interactionFactor * CANVAS_CONFIG.OPACITY_BOOST
      );
      dot.currentRadius =
        dot.baseRadius + interactionFactor * CANVAS_CONFIG.RADIUS_BOOST;
